...
namespace SuperMarketApi.Controllers
{
    ...
    public class CreditTrxController : Controller
    {
        ...
        public IActionResult GetCreditTrx([FromBody] dynamic objData)
         {
            try
            {
                ...
                int billType = jsonObj[0].billType;
        ...
        [HttpGet("getcreditdata")]
        public IActionResult getcreditdata(int searchId, int searchContactId, int billType, string billStatus, DateTime? dateFrom, DateTime? dateTo,
            int UserID, int numRecords, string reference, int contactType, float amountFrom, float amountUpto, string creditTypeStr, int companyId)
        {
            try
            {
                SqlConnection sqlCon = new SqlConnection(Configuration.GetConnectionString("myconn"));
        ...
        //Queen-14-09-2022
        [HttpGet("TestCreditData")]
        public IActionResult TestCreditData(int CompanyId)
        {
            try
            {
                SqlConnection sqlCon = new SqlConnection(Configuration.GetConnectionString("myconn"));
                ...
                SqlCommand cmd = new SqlCommand("dbo.Test_CreditIndex", sqlCon);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add(new SqlParameter("@CompanyId", CompanyId));
                DataSet ds = new DataSet();
                SqlDataAdapter sqlAdp = new SqlDataAdapter(cmd);
        ...
        public IActionResult PayCredit([FromBody] dynamic objData)
        {
            using (var scope = SQLExt.CreateTransScope())
            {
                ...
                catch (Exception e)
                {
                    ...
                    var data = new { error = new Exception(e.Message, e.InnerException), data = e.Message, msg = "Contact your service provider" };
                    return Json(data);
                }
            }
        }
